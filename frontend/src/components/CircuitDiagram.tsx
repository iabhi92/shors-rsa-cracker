...
export default function CircuitDiagram() {
  ...
  return (
    <svg viewBox="0 0 620 190" className="w-full" role="img" aria-label="Schematic of the Shor's algorithm circuit: Hadamards on control qubits, controlled modular exponentiation, inverse QFT, measurement">
      ...
      {wireY.map((y, i) => (
        <g key={i}>
          <line x1={70} y1={y} x2={550} y2={y} stroke="#1b2430" strokeWidth="1.5" />
          <text x={20} y={y + 4} fill="#8c919b" fontSize="11" fontFamily="var(--font-mono)">
            c{i}
          </text>
        </g>
      ))}
      <text x={20} y={wireY[2] + 22} fill="#8c919b" fontSize="10" fontFamily="var(--font-mono)">
        ⋮ n_count
      </text>

      {/* target wire */}
      <line x1={70} y1={targetY} x2={550} y2={targetY} stroke="#1b2430" strokeWidth="1.5" />
      <text x={12} y={targetY + 4} fill="#8c919b" fontSize="11" fontFamily="var(--font-mono)">
        |1⟩
      </text>
      ...
      {wireY.map((y, i) => (
        ...
        >
          <rect x={95} y={y - 14} width="28" height="28" rx="2" fill="#0b1018" stroke="#c99545" strokeWidth="1.5" />
      ...
      <motion.g initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
        {wireY.map((y, i) => (
          <circle key={i} cx={230} cy={y} r="4" fill="#eee8da" />
        ))}
        <line x1={230} y1={wireY[0]} x2={230} y2={targetY} stroke="#eee8da" strokeWidth="1.5" />
        <rect x={195} y={targetY - 16} width="70" height="32" rx="2" fill="#0b1018" stroke="#eee8da" strokeWidth="1.5" />
        <text x={230} y={targetY + 5} fill="#eee8da" fontSize="12" fontWeight="600" textAnchor="middle" fontFamily="var(--font-mono)">
          U_a^x
        </text>
      </motion.g>
      ...
      <motion.g initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }}>
        <rect x={340} y={wireY[0] - 20} width="90" height={wireY[2] - wireY[0] + 40} rx="2" fill="#0b1018" stroke="#54c89a" strokeWidth="1.5" />
      ...
      {wireY.map((y, i) => (
        ...
        >
          <rect x={475} y={y - 14} width="30" height="28" rx="2" fill="#0b1018" stroke="#8c919b" strokeWidth="1.5" />
          <path d={`M 481 ${y + 6} A 8 8 0 0 1 499 ${y + 6}`} stroke="#8c919b" strokeWidth="1.3" fill="none" />
          <line x1={490} y1={y + 6} x2={497} y2={y - 5} stroke="#8c919b" strokeWidth="1.3" />
        </motion.g>
      ))}
    </svg>
  )
